...
function RecommendationCarousel({ go }: { go: (view: MobileView) => void }) {
  ...
  return <div className="mobile-carousel-wrap"><div ref={scrollerRef} className="mobile-scroll-cards" onScroll={updateProgress}>{services.slice(0, 3).map(({ icon: Icon, title, tone }) => <button className="mobile-rec-card" key={title} onClick={() => go('book')}><div className={`mobile-rec-image ${tone}`}><Icon /></div><strong>{title}</strong><small>At your door</small></button>)}</div><div className="mobile-carousel-progress" aria-hidden="true"><span style={{ left: `${progress * 100}%` }} /></div></div>
}
